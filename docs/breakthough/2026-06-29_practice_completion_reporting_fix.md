...
- 채점 실패 시에도 세션 기록이 사라지지 않도록 `채점 실패` 복구 리포트를 생성한다.
- 복구 리포트에는 대화 기록, 신체진찰 기록, 오류 원문을 보존한다.
...
17. 채점 실패 시에는 `채점 실패` 복구 리포트가 생성되어 오류 원문과 기록이 남아야 함
...
이후 추가 보강으로 채점 API 실패 시에도 실패 복구 리포트를 생성해 기록 유실을 방지한다.
